...
static var burning_AI = func(_self:Obj_function ,delta):
	...
	if _self.timer_get(_self.AI_name + "cool", 0.0 ) == 0.0:
		...
		for obj in world.get_objs():
			if obj.get_addon_info("type") == "monster" and (obj.get_obj_position() - _self.get_obj_position()).length() < _self.get_item_info("dist",500.0):
				obj. call_handler("take_damage", _self.get_item_info("damage",{}))
		_self.timer_set(_self.AI_name + "cool", _self.get_item_info("cool_time",1.0) )
